...
class DictController extends CommonController {
	...
	public function index() {
		...
		$show       = $Page->show();
		$lists = $DictIndex->where($condstr)->order('id')->limit($Page->firstRow.','.$Page->listRows)->order("weight DESC")->select();
...
//		echo $show;exit;
	...
	public function other() {
		...
		$Page       = new \Think\Page($count, 15);
		$show       = $Page->show();
		$lists = $Dis->where($condstr)->order('id')->limit($Page->firstRow.','.$Page->listRows)->order("weight DESC")->select();
		$this->assign('lists', $lists);
		$this->assign('page', $show);
