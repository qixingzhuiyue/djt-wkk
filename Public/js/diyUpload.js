...
(function( $ ) {
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		var file_id = file.id;
		var $parentFileBox = $($fileInput.children('.parentFileBox').get(0));
		//添加父系容器;
		if ( $parentFileBox.length <= 0 ) {
			...
			$fileInput.prepend( div );
			$parentFileBox = $($fileInput.children('.parentFileBox').get(0));
		...
		$fileBox.on('click',function(){
			removeLi( $(this), file_id, webUploader );
		})
		$('li').on('mouseenter',function(){

			$(this).children('.diyCancel').css('display','block');
		}).on('mouseleave',function(){
			$(this).children('.diyCancel').css('display','none');
		})
